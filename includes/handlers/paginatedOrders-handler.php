...
if(isset($_SESSION['filterStatus']) && $_SESSION['filterStatus']!=0){
  $filterStatus = $_SESSION['filterStatus'];
  $query = mysqli_query($con, "SELECT * FROM `orders` 
        where supplierId = (SELECT id from retailer where emailid='$term')
...
$term = ucfirst($term);
    ...
    if(isset($_SESSION['filterStatus']) && $_SESSION['filterStatus']!=0){
      $filterStatus = $_SESSION['filterStatus'];
      $query2 = mysqli_query($con, "SELECT orders.orderId, brandname.brandName, manufacturer.manName, dosageform.formName, medicine.pack, user.name, orders.time, orders.quantity, orderstatus.status 
        FROM `orders` 
        ...
        where orders.supplierId = (SELECT id from retailer where emailid='$term') 
        and orders.status = '$filterStatus'
        order by orders.time desc $limit");
    }
    else{
      $query2 = mysqli_query($con, "SELECT orders.orderId, brandname.brandName, manufacturer.manName, dosageform.formName, medicine.pack, user.name, orders.time, orders.quantity, orderstatus.status 
        FROM `orders` 
