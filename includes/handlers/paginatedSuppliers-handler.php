...
$query = mysqli_query($con, "SELECT * FROM retailer");
// confirm($query);
...
// $query2 is what we will use to to display products with out $limit variable
    ...
    $query2 = mysqli_query($con, "SELECT name, emailid, phone, location, license, openingTime, closingTime, date FROM retailer $limit");
// confirm($query2);
...
// Remember we use query 2 below :)
  ...
  foreach($data as $d){
    ...
    $timings = substr($d['openingTime'], 0,5).' - '. substr($d['closingTime'], 0, 5);
    $date = $d['date'];
